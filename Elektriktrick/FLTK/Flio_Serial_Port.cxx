...
int Flio_Serial_Port::open(const char *portname, int bps)
{
    ...
    tio.c_cflag = (CS8 | CREAD | CLOCAL); // tfr is 8n1, not setting CLOCAL should enable DTR/DSR handshake
...
int Flio_Serial_Port::print(const char *text)
{
    return write((unsigned char*)text, (int)strlen(text));
}
...
int Flio_Serial_Port::write(const unsigned char *data, int n)
{
    ...
    int ret = (int)::write(port_, data, n);
    return ret;
#endif
}
...
void Flio_Serial_Port::reader()
{
    ...
    for (;;) {
        int n = free_to_end();
        int na = (int)::read(port_, ring_+ringHead_, n);
        if (na<=0)
            break;
#ifdef DEBUG_RECV
        else {
...
void Flio_Serial_Port::timeout_cb(void *d)
{
    Flio_Serial_Port *This = (Flio_Serial_Port*)d;
    This->pReadlnState = 2;
}


void Flio_Serial_Port::readln_cb(Fl_Widget *d, void *v)
{
    Flio_Serial_Port *This = (Flio_Serial_Port*)d;
    unsigned char EOL = (unsigned char)(intptr_t)v;
    // scan buffer for EOL
    if (This->available()) {
        if (This->ringHead_>This->ringTail_) {
            if (memchr(This->ring_+This->ringTail_, EOL, This->ringHead_-This->ringTail_))
                This->pReadlnState = 1;
        } else {
            if (memchr(This->ring_+This->ringTail_, EOL, This->NRing_-This->ringTail_))
                This->pReadlnState = 1;
            if (memchr(This->ring_, EOL, This->ringHead_))
                This->pReadlnState = 1;
        }
    }
}
...
int Flio_Serial_Port::readln(char *dest, int n, char EOL)
{
    // state=0: wait for EOL
    // state=1: EOL found
    // state=2: timeout

    // check if line is already available
    pReadlnState = 0;
    readln_cb(this, (void*)(intptr_t)EOL);
    // if there was no EOL yet, set the timer and wait for the EOL
    if (pReadlnState==0) {
        callback(readln_cb, (void*)(intptr_t)EOL);
        Fl::add_timeout(pTimeout, timeout_cb, this);
        while (pReadlnState==0) {
            Fl::wait(pTimeout/10);
        }
        callback(readln_cb, 0);
    }
    // did a timeout occur?
    if (pReadlnState==2) {
        return 0;
...
void Flio_Serial_Port::timeout(double t)
{
    pTimeout = t;
}
